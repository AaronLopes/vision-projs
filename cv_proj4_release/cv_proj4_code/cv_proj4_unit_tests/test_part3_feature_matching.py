...
from student_code import compute_feature_distances, match_features_ratio_test
...
def test_compute_feature_distances_2d():
    ...
    inter_dists = compute_feature_distances(torch.from_numpy(feats1).float(), torch.from_numpy(feats2).float()).numpy()
...
def test_compute_feature_distances_10d():
    ...
    inter_dists = compute_feature_distances(torch.from_numpy(feats1), torch.from_numpy(feats2)).numpy()
...
def test_match_features_ratio_test():
    ...
    result, confidences = match_features_ratio_test(torch.from_numpy(feats1).float(), torch.from_numpy(feats2).float())
    result = result.numpy()
